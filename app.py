    ...
    for i, flight in enumerate(sorted_flights):
        with st.expander(f"{flight.get('airline', 'Airline')} - {flight.get('from', 'Origin')} to {flight.get('to', 'Destination')} - {flight.get('price', 'Price')}" + (" 🏆 Best Value!" if i == 0 else "")):
            ...
            with col1:
                ...
                image_url = flight.get("image", flight.get("imag", default_flight_image))
                    ...
                    if not layover_details or len(layover_details) == 0:
                        ...
                        for layover in layover_details:
                            ...
                            st.markdown(f"""<div style='background-color: #f8f9fa; padding: 10px; border-radius: 5px; margin-bottom: 5px;'>
                                <strong>🏙️ {city}</strong> - ⏱️ {duration}<br>
    ...
    for i, hotel in enumerate(sorted_hotels):
        with cols[i % 3]:
            with st.container():
                ...
                image_url = hotel.get("image", default_hotel_image)
                if not image_url or image_url.strip() == "":
                    image_url = default_hotel_image
                # Decrease image size by setting width to 150 pixels instead of full column width
